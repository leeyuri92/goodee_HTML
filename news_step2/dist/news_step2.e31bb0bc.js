...
var newsList = JSON.parse(ajax.response); //위 요청을 서버에서 받아오는 속성 response
console.log(newsList); // 서버측에서 보낸 응답 출력해보기 - Array로 변화 출력

var ul = document.createElement('ul'); // DOM API 가지고 태그를 만드니까 직관적이지 않다. DOM Tree 그려지지 않아서
...
for (var i = 0; i < 5; i++) {
  ...
  a.innerHTML = "".concat(newsList[i].title, "(\u2764\uFE0F").concat(newsList[i].comments_count, ")");
  li.appendChild(a);
  ul.appendChild(li);
}
